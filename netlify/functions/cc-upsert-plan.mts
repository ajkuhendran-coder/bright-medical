...
export default async (req: Request, _context: Context) => {
  ...
  // Version++ (bestehenden Plan lesen)
  let nextVersion = 1
  try {
    ...
    console.error('[cc-upsert-plan] version lookup failed', err)
    return jsonResponse(500, { error: 'Plan konnte nicht gelesen werden.' })
  }

  const row = { client_sub: email, subject_id: subjectId, title, intro, sections, version: nextVersion, updated_at: new Date().toISOString() }
  try {
    const res = await fetch(`${creds.url}/rest/v1/portal_plans?on_conflict=client_sub`, {
      method: 'POST',
      headers: {
        apikey: creds.serviceKey, Authorization: `Bearer ${creds.serviceKey}`,
        'Content-Type': 'application/json', Prefer: 'resolution=merge-duplicates,return=minimal',
      },
      body: JSON.stringify(row),
    })
    if (!res.ok) {
      console.error('[cc-upsert-plan] upsert failed', res.status, await res.text())
      return jsonResponse(500, { error: 'Plan konnte nicht gespeichert werden.' })
    }
  } catch (err) {
    console.error('[cc-upsert-plan] upsert error', err)
    return jsonResponse(500, { error: 'Plan konnte nicht gespeichert werden.' })
  }
